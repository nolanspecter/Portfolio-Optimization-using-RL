...
class CardGameEnv(py_environment.PyEnvironment):
    ...
    def __init__(self):
        self._action_spec = array_spec.BoundedArraySpec(
            (len(conf.COINS)+1,), np.float64, minimum=0, maximum=1, name='action')
        self._observation_spec = array_spec.BoundedArraySpec(
            shape=(len(conf.OBS_COLS),), dtype=np.float64, minimum=conf.OBS_COLS_MIN,\
                maximum=conf.OBS_COLS_MAX,\
                     name='observation')
        self.reset()
    ...
    def _reset(self):
        self.memory_return = pd.DataFrame(columns=[t+"_close" for t in conf.COINS])
        self._episode_ended = False
        self.index = 0
        ...
        self.money_split_ratio = np.zeros((len(conf.COINS)+1))
        self.money_split_ratio[0] = 1

        self.df = pd.read_csv(conf.FILE)
        self.scaler = preprocessing.StandardScaler()
        
        self.df["date"] = self.df["date"].apply(lambda x: pd.Timestamp(x, unit='s', tz='US/Pacific'))
        self.df = self.df[self.df["coin"].isin(conf.COINS)].sort_values("date")
        self.scaler.fit(self.df[conf.SCOLS].values)
        self.df = self.df.reset_index(drop=True)

        self.max_index = self.df.shape[0]
        start_point = (np.random.choice(np.arange(3,self.max_index - conf.EPISODE_LENGTH))//3) *3
        end_point = start_point + conf.EPISODE_LENGTH//3 *3
        ...
        self.dfslice = self.df[(self.df["coin"].isin(conf.COINS))&(self.df["date"]>=self.current_time)&(self.df["date"]<self.current_time+pd.Timedelta(5,unit='m'))].copy().drop_duplicates("coin")
        ...
        self._state = info_["scaled_output"][conf.OBS_COLS].values.flatten()
        reward = info_["reward"]
        self._episode_ended = True if self.index==conf.EPISODE_LENGTH//3 else False
    ...
    def get_observations(self):
        ...
        for i,grp in dfslice.groupby("coin"):
            tempdf = pd.DataFrame(self.scaler.transform(grp[conf.SCOLS].values))
            tempdf.columns = [i+"_"+c for c in conf.SCOLS]
            if dfs.empty:
                dfs = tempdf
            else:
    ...
    def get_observations_unscaled(self):
        ...
        dfs = pd.DataFrame()
        for i,grp in dfslice.groupby("coin"):
            tempdf = pd.DataFrame(grp[conf.COLS].values)
            tempdf.columns = [i+"_"+c for c in conf.COLS]
            if dfs.empty:
                dfs = tempdf
            else:
                dfs = dfs.merge(tempdf,right_index=True,left_index=True,how='inner')
        
        self.memory_return = pd.concat([self.memory_return,dfs[[t+"_close" for t in conf.COINS]]],ignore_index=True)
    ...
    def calculate_actual_shares_from_money_split(self):
        ...
        num_shares = []
        for i,c in enumerate(conf.COINS):
